let trackList = [
    {
        name: "Mezzo Mondo",
        path: "./Assest/Music/Emma-Mezzo Mondo.mp3",
        img: "./Assest/Images/Emma.jpeg",
        singer: "Emma",
    },
    {
        name: "Idem",
        path: "./Assest/Music/Gazzelle - IDEM.mp3",
        img: "./Assest/Images/Idem.jpeg",
        singer: "Gazzelle",
    },
    {
        name: "Se poi domani",
        path: "./Assest/Music/LDA - Se poi domani (Testo Lyrics).mp3",
        img: "./Assest/Images/Lda.jpeg",
        singer: "Lda",
    },
    {
        name: "Mare fuori",
        path: "./Assest/Music/Ddoje mane (feat. Raiz).mp3",
        img: "./Assest/Images/Mare_fuori.png",
        singer: "Raiz",
    },
    {
        name: "Destinazione Mare",
        path: "./Assest/Music/Tiziano Ferro - Destinazione Mare (Official Video).mp3",
        img: "./Assest/Images/Destinazione_mare.jpeg",
        singer: "Tiziano Ferro",
    }
];
